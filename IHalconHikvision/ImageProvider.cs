...
namespace IHalconHikvision
{
    public class ImageProvider
    {
        ...
        private void ImageCallbackFunc(IntPtr pData, ref MyCamera.MV_FRAME_OUT_INFO stFrameInfo, IntPtr pUser)
        {
            ...
            ImgWidth = mImage.mWidth;
            ImgHeight = mImage.mHeight;
        ...
        public void SetROIXYWidthHeight(uint w_offset, uint h_offset, uint w, uint h)
        {
            try
            {
                if (m_open)
                {
                    m_pMyCamera.MV_CC_SetIntValue_NET("OffsetX", 0);
                    m_pMyCamera.MV_CC_SetIntValue_NET("OffsetY", 0);
                    Thread.Sleep(50);
                    m_pMyCamera.MV_CC_SetIntValue_NET("Width", w);
                    m_pMyCamera.MV_CC_SetIntValue_NET("Height", h);
                    m_pMyCamera.MV_CC_SetIntValue_NET("OffsetX", w_offset);
                    m_pMyCamera.MV_CC_SetIntValue_NET("OffsetY", h_offset);
                }
            }
            catch (Exception ex)
            {
        ...
        public void ExposureSource(uint NUM)
        {
            // 0  时间曝光   1 脉宽曝光
            if (NUM >= 0 && NUM <= 1)
                m_pMyCamera.MV_CC_SetEnumValue_NET("ExposureMode", NUM);
            else
                throw new Exception("触发源必须是0或者1!");
        }

        public void TrigPolarity(uint NUM)
        {
                //0:RisingEdge
                //1:FallingEdge
                //2.LevelHigh
            ...
            // 0  时间曝光   1 脉宽曝光
            if (NUM >= 0 && NUM <= 3)
                m_pMyCamera.MV_CC_SetEnumValue_NET("TriggerActivation", NUM);
            else
                throw new Exception("触发源必须是0或者1!");
        }
        ...
        public void GetCurrentImage(ref Bitmap ho_Image)
        {
            lock (m_lockObject) /* Lock the grab result queue to avoid that two threads modify the same data. */
            {
                if (m_grabbedBuffers.Count > 0) /* If images available. */
                {
                    Ho_ImageFactory.Createho_24Image(out ho_Image, m_grabbedBuffers[0].Buffer, m_grabbedBuffers[0].Width, m_grabbedBuffers[0].Height, m_grabbedBuffers[0].Color);
                    ReleaseImage();
                    return;
                }
            }
